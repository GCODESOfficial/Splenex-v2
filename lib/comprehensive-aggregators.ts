...
async function getLiFiQuote(request: QuoteRequest): Promise<UnifiedQuote | null> {
  try {
    console.log("[LiFi] 🌉 Getting quote from LiFi with fee collection...");
    
    if (!LIFI_API_KEY) {
      console.log("[LiFi] ⚠️ No API key configured");
      return null;
    }
    ...
    console.log(`[LiFi] Token normalization: ${request.fromToken} → ${normalizedFromToken}`);
    console.log(`[LiFi] Token normalization: ${request.toToken} → ${normalizedToToken}`);
      ...
      if (errorData.message?.includes("not configured for collecting fees")) {
        ...
        if (retryResponse.ok) {
          ...
          if (retryData.estimate) {
            console.log(`[LiFi] ✅ Quote received without fee collection`);
            return {
              provider: "lifi",
    ...
    if (!quoteData.estimate) {
      console.log("[LiFi] ❌ No estimate in response");
      return null;
    }

    console.log(`[LiFi] ✅ Quote received with 2% fee collection`);
    console.log(`[LiFi] 💰 Integrator: SPLENEXDAPP`);
    console.log(`[LiFi] 💰 Fee rate: 2%`);
...
async function get1inchQuote(request: QuoteRequest): Promise<UnifiedQuote | null> {
  try {
    console.log("[1inch] 🔥 Getting quote from 1inch...");
    
    if (!ONEINCH_API_KEY) {
      console.log("[1inch] ⚠️ No API key configured");
      return null;
    }

    const chainName = CHAIN_ID_MAP[request.fromChain];
    if (!chainName) {
      console.log(`[1inch] ❌ Chain ${request.fromChain} not supported`);
      return null;
    }
    ...
    if (!response.ok) {
      console.log(`[1inch] ❌ Quote API failed: ${response.status}`);
      return null;
    }
...
async function get0xQuote(request: QuoteRequest): Promise<UnifiedQuote | null> {
  try {
    console.log("[0x] ⚡ Getting quote from 0x Protocol...");
    
    if (!ZEROX_API_KEY) {
      console.log("[0x] ⚠️ No API key configured");
      return null;
    }
    ...
    if (!response.ok) {
      console.log(`[0x] ❌ Quote API failed: ${response.status}`);
      return null;
    }
...
async function getParaswapQuote(request: QuoteRequest): Promise<UnifiedQuote | null> {
  try {
    console.log("[Paraswap] 🦋 Getting quote from Paraswap...");
    ...
    if (!response.ok) {
      console.log(`[Paraswap] ❌ Quote API failed: ${response.status}`);
      return null;
    }
