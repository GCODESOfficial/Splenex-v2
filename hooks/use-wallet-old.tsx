...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchAllTokenBalances = async (walletAddress: string, currentChainId: string) => {
    try {
      ...
      // NEW: Use comprehensive detection for ALL chains
      try {
        console.log("[v0] 🚀 Using comprehensive detection for ALL chains...")
        const detector = new ComprehensiveTokenDetector()
        
        // Get all supported chains
        const chainKeys = Object.keys(SUPPORTED_CHAINS) as ChainKey[]
        console.log("[v0] 📋 Processing all chains:", chainKeys.map(cId => SUPPORTED_CHAINS[cId].name))
        
        // Process all chains in parallel for faster results
        const chainPromises = chainKeys.map(async (cId) => {
          const chainConfig = SUPPORTED_CHAINS[cId]
          try {
            console.log(`[v0] 🔍 Comprehensive detection for ${chainConfig.name} (${chainConfig.moralisChain})...`)
            
            const result = await detector.detectAllTokens(walletAddress, chainConfig.moralisChain)
            
            console.log(`[v0] ✅ ${chainConfig.name}: Found ${result.tokens.length} tokens`)
            console.log(`[v0] 💰 ${chainConfig.name}: Total USD value: $${result.totalUsdValue.toFixed(2)}`)
            console.log(`[v0] 🔍 ${chainConfig.name}: Methods used: ${result.detectionMethods.join(', ')}`)
            
            // Convert to our format with proper chain name
            ...
            console.log(`[v0] ❌ Comprehensive detection failed for ${chainConfig.name}:`, error)
            return []
          }
        })
        ...
        // Flatten all results
        allChainResults.forEach((chainTokens, index) => {
          const chainName = SUPPORTED_CHAINS[chainKeys[index]].name
          console.log(`[v0] 📋 ${chainName} tokens:`, chainTokens.map(t => `${t.symbol}: ${t.balance}`))
          allTokenBalances.push(...chainTokens)
        })
        
        console.log(`[v0] 🎉 TOTAL: Found ${allTokenBalances.length} tokens across all chains`)
        console.log(`[v0] 💰 TOTAL USD value: $${allTokenBalances.reduce((sum, t) => sum + t.usdValue, 0).toFixed(2)}`)
        
        // Detailed logging of all tokens found
        console.log(`[v0] 📋 DETAILED TOKEN LIST:`)
        allTokenBalances.forEach((token, index) => {
        ...
        console.log("[v0] ❌ Comprehensive detection failed, falling back to original method:", error)
      }

      // No fallback - Covalent GoldRush is the only method
      if (allTokenBalances.length === 0) {
        console.log("[v0] ⚠️ Comprehensive detection found no tokens - Covalent GoldRush is the only method")
        console.log("[v0] 🔍 Check Covalent API key and network connectivity")
      }
              ...
              if (data.result && data.result.length > 0) {
                ...
                if (tokensWithBalance.length > 0) {
                  ...
                  tokensWithBalance.forEach((token: any) => {
                    ...
                    console.log(`[v0] Processing token ${token.symbol}: raw=${token.balance}, decimals=${decimals}, calculated=${bal}, threshold=0.000000000001`)

                    if (bal > 0) { // No threshold - detect ALL tokens regardless of amount
                      ...
                      allTokenBalances.push({
                        ...
                        balance: bal.toFixed(12), // Show more decimal places for small amounts
                        usdValue,
                        price,
      ...
      } // End of fallback if statement

      // Calculate total USD value from all tokens
      const totalUsdValue = allTokenBalances.reduce((sum, token) => sum + (token.usdValue || 0), 0)

      setTokenBalances(allTokenBalances)
      setTotalUsdBalance(totalUsdValue)
