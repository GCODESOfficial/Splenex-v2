...
export function ForexTradingInterface({ onSwitchToCrypto }: ForexTradingInterfaceProps) {
  ...
  useEffect(() => {
    const fetchForexData = async () => {
      try {
        ...
        console.error("[v0] Forex data fetch error:", error)
      }
    }
  ...
  const calculateMargin = () => {
    ...
    return notionalValue / leverage[0]
  }
  ...
  const closePosition = async (positionId: string) => {
    ...
    try {
      ...
      console.error("[v0] Position close error:", error)
      ...
      <div className="flex justify-center pt-4 pb-2">
        <div className="flex bg-gray-900 rounded-lg p-1">
          <Button
            ...
            className="bg-transparent text-gray-400 hover:text-white hover:bg-gray-800 px-6 py-2"
          >
            Crypto
          </Button>
          <Button className="bg-yellow-400 text-black hover:bg-yellow-500 px-6 py-2" disabled>
            Forex/Trade
          </Button>
        </div>
      </div>
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-6 text-sm">
                  <div>
                    ...
                    <div className="font-mono">${selectedPair.price.toFixed(4)}</div>
                  </div>
                  <div>
                    <span className="text-gray-400">24H Change</span>
                    <div
                      className={`flex items-center ${selectedPair.change24h >= 0 ? "text-green-400" : "text-red-400"}`}
                    >
                  ...
                  <div>
                    ...
                    <div className="font-mono">${selectedPair.high24h.toFixed(4)}</div>
                  </div>
                  <div>
                    <span className="text-gray-400">24H Low</span>
                    <div className="font-mono">${selectedPair.low24h.toFixed(4)}</div>
                  </div>
                  <div>
                    <span className="text-gray-400">Funding</span>
                    <div className="font-mono">{(selectedPair.funding * 100).toFixed(4)}%</div>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="bg-gray-800 rounded-lg h-96">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={priceData}>
                    ...
                    <YAxis stroke="#9CA3AF" fontSize={12} domain={["dataMin - 0.001", "dataMax + 0.001"]} />
                    <Tooltip
                      contentStyle={{
                    ...
                    <Line
                      type="monotone"
                      dataKey="price"
                      stroke={selectedPair.change24h >= 0 ? "#10B981" : "#EF4444"}
                      strokeWidth={2}
                      dot={false}
                    />
                  </LineChart>
          ...
          <Card className="bg-gray-900 border-gray-700 h-full">
            <div className="p-4 space-y-4">
              ...
              <div className="flex space-x-2">
                <Button
                  onClick={() => {
                    console.log("[v0] Switching to market order type")
                    setOrderType("market")
                  }}
                ...
                <Button
                  onClick={() => {
                    console.log("[v0] Switching to limit order type")
                    setOrderType("limit")
                  }}
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-white">
                    {amount ? (Number.parseFloat(amount) * selectedPair.price).toFixed(2) : "-"}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-1 text-xs text-gray-400 border-t border-gray-700 pt-4">
                ...
                <div className="flex justify-between">
                  ...
                  <span
                    className={
                      positions.reduce((total, pos) => total + pos.pnl, 0) >= 0 ? "text-green-400" : "text-red-400"
                    }
                  >
                    {positions.reduce((total, pos) => total + pos.pnl, 0).toFixed(2)} USD
                  </span>
                </div>
              </div>
            </div>
